...
class Duk extends MY_Controller
{

	function __construct()
	{
		parent::__construct();
		$this->load->model(array('Mod_duk'));
	}
	...
	public function ajax_list()
	{
		...
		set_time_limit(3600);
		$list = $this->Mod_duk->get_datatables();
		$data = array();
		$no = $_POST['start'];
		foreach ($list as $pel) {
			...
			$row[] = $pel->nama_pangkat.' - '.$pel->jenis_pangkat;
			$data[] = $row;
		}

		$output = array(
			"draw" => $_POST['draw'],
			"recordsTotal" => $this->Mod_duk->count_all(),
			"recordsFiltered" => $this->Mod_duk->count_filtered(),
			"data" => $data,
		);
	...
	public function insert()
	{
		...
		$this->Mod_duk->insert_pegawai("tbl_pegawai", $save);
	...
	public function update()
	{
		...
		$this->Mod_duk->update_pegawai($nip, $save);
		echo json_encode(array("status" => TRUE));
	}

	public function edit_pegawai($nip)
	{
		$data = $this->Mod_duk->get_pegawai($nip);
	...
	{
		$data['pegawai'] = $this->Mod_duk->get_by_id($nip);
		$data['data_keluarga'] = $this->Mod_duk->get_keluarga($nip);
		$data['data_pendidikan'] = $this->Mod_duk->get_pendidikan($nip);
		$data['data_jabatan'] = $this->Mod_duk->get_jabatan($nip);
		$data['data_pangkat'] = $this->Mod_duk->get_pangkat($nip);
		$data['data_kgb'] = $this->Mod_duk->get_kgb($nip);
